...
def standarize_fft(signal, fft, scale_bool = False):
    opt_scale = 0.05
    if scale_bool: scale = scale_bool
    else: scale = signal.axes_manager['x'].scale
...
def noise_model(fft_image,real_image,s,theta,sigma,lamda,hd,vd,scale, total_pixels,resolution = 6,intensity = 50):
    ...
    real_image_normalized = np.abs(real_image)
        ...
        for eje in ejes:
            ...
            for pixel_size in size_pixel:
                ...
                for i in range(size):
                    ...
                    print('Simulación de la imagen: ',j+1, ' de ', total, ' en ', tiempo)   
                    j += 1
